...
class _FixtureListScreenState extends ConsumerState<FixtureListScreen> {
  ...
  String _getEncounterText(int count) {
    switch (count) {
      case 1:
        return "1er Encuentro";
      case 2:
        return "2do Encuentro";
      case 3:
        return "3er Encuentro";
      case 4:
        return "4to Encuentro";
      case 5:
        return "5to Encuentro";
      case 6:
        return "6to Encuentro";
      default:
        return "$countº Encuentro";
    }
  }
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              "Elige cómo deseas programar los partidos de este torneo.",
            ...
            ListTile(
              ...
              subtitle: const Text(
                "Crea todas las jornadas y cruza a todos los equipos usando el método de Round Robin.",
            ...
            ListTile(
              ...
              leading: const CircleAvatar(
                ...
                child: Icon(Icons.edit_calendar, color: Colors.black),
              ),
              title: const Text(
                "Constructor Manual",
                style: TextStyle(
              ...
              subtitle: const Text(
                "Crea jornadas y programa partidos uno por uno, ideal para torneos en progreso.",
                style: TextStyle(color: Colors.white54, fontSize: 12),
              ),
              onTap: () {
                Navigator.pop(ctx);
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (_) => ManualFixtureBuilderScreen(
  ...
  void _showRoundsFilterBottomSheet(
    List<String> rounds,
    String currentActiveRound,
  ) {
    showModalBottomSheet(
      context: context,
      backgroundColor: Colors.transparent,
  ...
  Future<void> _generateNewFixture(BuildContext context) async {
    ...
    try {
      ...
      if (existing.isNotEmpty) {
        final purge = await api.deleteFixture(
          tournamentId: widget.tournamentId,
        );
      ...
      // 3. Generar
      final result = await api.generateFixture(
        tournamentId: widget.tournamentId,
      );
  ...
  Future<void> _openFinishedMatch(Fixture match) async {
    final prepared = await ref.read(finishedMatchLoaderProvider)(match);
    if (!mounted) return;
      ...
      body: AppBackground(
        ...
        child: fixtureAsync.when(
          ...
          data: (groupedRounds) {
            ...
            allMatches.sort((a, b) {
              int roundA =
                  int.tryParse(a.roundName.replaceAll(RegExp(r'[^0-9]'), '')) ??
                  0;
              int roundB =
                  int.tryParse(b.roundName.replaceAll(RegExp(r'[^0-9]'), '')) ??
                  0;
                      ...
                      children: [
                        ...
                        Material(
                          ...
                          child: InkWell(
                            ...
                            onTap: () => _showRoundsFilterBottomSheet(
                              allRounds,
                              activeRound,
                            ),
                            child: Container(
  ...
  Widget _buildTeamLogo(String? logoPath, String teamName) {
    final String initial = teamName.isNotEmpty
  ...
  Widget _buildMatchCard(
    ...
    if (match.scheduledDatetime != null) {
      ...
      dateFormatted =
          "${dt.day.toString().padLeft(2, '0')}/${dt.month.toString().padLeft(2, '0')}/${dt.year}  •  ${dt.hour.toString().padLeft(2, '0')}:${dt.minute.toString().padLeft(2, '0')} hrs";
    }
            ...
            child: InkWell(
              onTap: isFinished
                  ...
                  ? () async {
                      ...
                      if (currentState.matchId.isNotEmpty &&
                          currentState.matchId != idABuscar) {
                      ...
                      final localMatch =
                          await (dbBase.select(dbBase.matches)
                                ..where((t) => t.id.equals(idABuscar)))
                              .getSingleOrNull();
                      ...
                      if (localMatch != null &&
                        ...
                        final tournamentData =
                            await (dbBase.select(dbBase.tournaments)..where(
                                  (t) => t.id.equals(
                                    localMatch.tournamentId ?? '',
                                  ),
                                ))
                                .getSingleOrNull();

                        final allDbPlayers = await dbBase
                            .select(dbBase.players)
                            .get();
                        final rosterA = allDbPlayers
                            .where((p) => p.teamId.toString() == match.teamAId)
                            .map(
                              (p) => CatalogPlayer(
                        ...
                        final rosterB = allDbPlayers
                            .where((p) => p.teamId.toString() == match.teamBId)
                            .map(
                              (p) => CatalogPlayer(
                                id: int.tryParse(p.id) ?? -1,
                                name: p.name,
                        ...
                        final dbRosters = await (dbBase.select(
                          dbBase.matchRosters,
                            ...
                            Navigator.push(
                              ...
                              MaterialPageRoute(
                                builder: (_) => MatchControlScreen(
                                  ...
                                  tournamentName:
                                      tournamentData?.name ?? "Torneo Activo",
                                  categoryName:
                                      tournamentData?.category ?? "LIBRE",
                                  // LOGOS RECUPERADOS EXITOSAMENTE
                                  tournamentLogoUrl:
                                      tournamentData?.logoUrl ?? "",
                                  refereeLogoUrl:
                                      tournamentData?.refereeLogoUrl ?? "",
                                  ...
                                  tournamentId:
                                      int.tryParse(
                                        localMatch.tournamentId ?? '0',
                                      ) ??
                                      0,
                                  venueId:
                                      int.tryParse(localMatch.venueId ?? '0') ??
                                      0,
                                  teamAId: localMatch.teamAId ?? 0,
                                  teamBId: localMatch.teamBId ?? 0,
                                  coachA: '',
                                  coachB: '',
                                  captainAId: capA != null
                                      ? int.tryParse(capA.playerId)
                                      : null,
                                  captainBId: capB != null
                                      ? int.tryParse(capB.playerId)
                                      : null,
                                ),
                              ),
                            ),
